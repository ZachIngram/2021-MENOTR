function [param_vals,best_chisq,chi_values,RMSD,indvar1_sim,simvals] = ga_fitting_routine_MC(params,generations, ...
    pop_size,data,weights,crossover,stdevs,best,mut_probability,elite,decay,nlls_size,indvar,CyMat,zy,gzy, ...
    params_ub,params_lb,kcy,CyMatH,Period,ORnum,algsw,axspace,run_fig)
...
%% INITIAL SETUP FOR A GENERATION:
    ...
    switch axspace
        case 'lin' % linear spacing
            indvar1_sim = linspace(min(indvar),max(indvar),1e3)';
        case 'log' % log spacing
            indvar1_sim = logspace(log10(min(indvar)),log10(max(indvar)),1e3)';
    end
...
running_chi = zeros(generations,1);
    % These lines generate sub-matrices for the mutation and crossover
...
%% MAIN GENERATION LOOP
    ...
    % where generation N refers to the generation number and generation T
...
% disp(['Period:',s_y,'   ','OptRoutine:',or_y,'   ','Cycle:',c_y,'   ','Generation:',g_y])    
    ...
    parfor q = 1:pop_size
        chisq(q,1) = sum(sum(((data-model_simulator(population_mat(q,1:end)',indvar))./weights).^2));  %#ok<PFBNS>
    end
    ...
    if nlls_size > 0
        ...
        if CyMat(length(CyMat)) == 1   
            parfor r = 1:nlls_size
                ...
                [fit_parlim,~,~,~] = lsqnl_fitting_routine(indvar,data,loop_lim,weights,algsw);
...
%         load('CurrentParamVals')
    ...
    residual = data-model_simulator(param_vals',indvar);
    RMSD = sqrt(sum(sum(residual.^2))/numel(data));
    VAR = sum(sum(residual.^2))/numel(data);
    simvals = model_simulator(param_vals',indvar);
    ...
    %% PLOTTING AND RUNNING WINDOW
        ...
        %displays which parameters are currently being floated(parameters
