function [param_vals,best_chisq,chi_values,RMSD,indvar1_sim,simvals] = ga_fitting_routine(params,generations, ...
...
disp(['Period:',s_y,'   ','OptRoutine:',or_y,'   ','Cycle:',c_y,'   ','Generation:',g_y])    
    ...
    if g>1
     ...
     %% Recalculation of CHI-SQUARED AND SORTING OF CHI-SQUARED   
        % Chisq values are calculated in parallel in the following parfor loop.
    parfor q = 1:pop_size
        chisq(q,1) = sum(sum(((data-model_simulator(population_mat(q,1:end)',indvar))./weights).^2));     
    end
    ...
    simvals = model_simulator(param_vals',indvar1_sim);
    ...
    switch run_fig
        ...
        hold on
        semilogx(indvar1_sim,simvals,'k')
        title('Data/Fits');
        ylabel('Signal')
        set(gca,'fontsize',20,'linewidth',2);
        set(gca,'fontsize',20,'linewidth',2);
