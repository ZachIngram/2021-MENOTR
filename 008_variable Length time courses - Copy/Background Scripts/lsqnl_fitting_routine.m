function [parlim,residual,resnorm,params,chisq,RMSD]=lsqnl_fitting_routine(indvar,data,parlim,weights,algsw,flag)
...
OPT = optimset(OPT,'UseParallel',true);
...
switch algsw
    case 0
        [x,resnorm,wres,~,~,~,~]=lsqnonlin('residual_calculator',parlim(:,3),parlim(:,2),parlim(:,4),OPT,parlim,data,indvar,weights,flag);    
    case 1
        [x,resnorm,wres,~,~,~,~]=lsqnonlin('residual_calculator',parlim(:,3),[],[],OPT,parlim,data,indvar,weights,flag);
end
