    ...
    for i = 1:ORnum
        ...
        if CyMatH > 1 
            ...
            zy = 0;% Counting index for user display 
                [param_vals(k+1,:)] ...
                    = ga_fitting_routine(param_vals(k,:),g(k),pop_size,data_input,weights,...
                crossover,stdevs,best,mut_probability,elite,decay,nlls_size,indvar,CyMat(k,:),...
            zy,g(k),params_ub,params_lb,k,CyMatH,Period,i,algsw,axspace,run_fig,flag);
            end
            zy = 1; k = 1;% Counting index for user display                
                [param_vals(CyMatH+1,:),best_chisq,chi_values,RMSD,indvar1_sim,simvals,flag_sim]...
                ...
                zy,g(CyMatH),params_ub,params_lb,k,CyMatH,Period,i,algsw,axspace,run_fig,flag);
        else
            zy = 1; k = 1;% Counting index for user display              
                [param_vals(CyMatH+1,:),best_chisq,chi_values,RMSD,indvar1_sim,simvals,flag_sim]...
                    = ga_fitting_routine(param_vals(CyMatH,:),g(CyMatH),pop_size,data_input,weights,...
...
% End of OR loop
        ...
        % indexes them
    for n = 1:ORnum
        chisq_or(n,1) = sum(sum(((data_input-model_simulator(param_vals_or(n,:),indvar,flag))./weights).^2));
    end
    ...
    %% Stopping criteria:
        ...
        % deviation) between the different optimization routine chi-squared 
...
Sim_Fits = model_simulator(param_vals_fit,indvar1_sim,flag_sim);
Sim_IndVar = indvar1_sim;
Sim_Data = zeros(size(Sim_IndVar,1),(size(Sim_IndVar,2)+size(Sim_Fits,2)));
Sim_Data(:,1) = Sim_IndVar;
Sim_Data(:,2:end) = Sim_Fits;
Residual = data_input-model_simulator(param_vals_fit,indvar,flag);
