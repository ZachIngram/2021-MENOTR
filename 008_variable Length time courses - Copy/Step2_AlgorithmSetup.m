...
Data_Set_1(:,4) = 1*ones(size(Data_Set_1,1),1);
Data_Set_2(:,4) = 2*ones(size(Data_Set_2,1),1);
Data_Set_3(:,4) = 3*ones(size(Data_Set_3,1),1);

InsertData = [Data_Set_1; Data_Set_2; Data_Set_3];
...
k = 5;                  % Initial guess
k_ub = 1000;            % Upper bound
k_lb = 0.01;            % Lower bound
...
param_vals = [k m Amp1 Amp2 Amp3];
params_ub = [k_ub m_ub Amp1_ub Amp2_ub Amp3_ub];
params_lb = [k_lb m_lb Amp1_lb Amp2_lb Amp3_lb];
clear k m k_ub m_ub k_lb m_lb
...
g(1)=3;     % g(1) is the first element in the generations matrix which  
            % corresponds to the number of generations to be performed 
            % in the first cycle, 3 indicates how many generations to 
            % perform within the first cycle. 
            
k_c(1) = 1;   % m_c(1) : m is the parameter name, _c indicates the cycles 
...
g(2)=3;     % g(2) is the second element in the generations matrix which 
            % corresponds to the number of generations to be performed in 
            % the second cycle and 3 indicates how many generations to  
            % perform within the second cycle. 
...
CyMat=[k_c; m_c;Amp1_c; Amp2_c; Amp3_c;NLLS]';
CyMatH=size(CyMat,1);
clear k_c m_c Amp1_c Amp2_c Amp3_c NLLS
...
data_input  = InsertData(:,2); %pulls out data
weights = InsertData(:,3); %pulls out weights
flag = InsertData(:,4);  
